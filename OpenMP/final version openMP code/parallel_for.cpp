...
template<int phase>
inline void calcSegment(const int bound_s_x, const int bound_e_x, const int bound_s_y, const int bound_e_y, double *tsqrerror)
{
    ...
    for(int i=bound_s_y; i<bound_e_y; i++)
    {
        for(int j=bound_s_x; j<bound_e_x; j++)
        {
            if ( (i+j)%2 == phase )
            {
                ...
                // tsqrerror += pow2(old_val - ro_u.get(i, j));
                tsqrerror[omp_get_thread_num()] += pow2(old_val - ro_u.get(i, j));
...
//printf("!!!%f,%f max=%f u=%f w=%f\n", x, y, max1, u.get(i, j), w.get(i, j));
    ...
    double ts_stop;
    #pragma omp master
    ts_stop = getElapsedtime(ts_start);

    double reorderFwTime = ts_stop/1000.0f;
...
//    printf("Thread %d dimensions %d x %d, data block %d x %d, last offset x=%d y=%d\n", omp_get_thread_num(), sx, sy, bs_x, bs_y, lastoffset_x, lastoffset_y);
    int start_pos_x = 1+bs_x * (0%sx),
        start_pos_y = 1+bs_y * (0/sx),
        end_pos_x   = start_pos_x+bs_x+(0%sx==sx-1?lastoffset_x:0),
        end_pos_y   = start_pos_y+bs_y+(0/sx==sy-1?lastoffset_y:0);
...
//#pragma omp barrier
    ...
    #endif*/
    sqrerror = 0.0;
    double tsqrerror[omp_get_num_threads()];
    #pragma omp parallel
    {
        do
        {
            ...
            {
                tsqrerror[omp_get_thread_num()] = 0;
            }
...
//printf("end of %d\n", omp_get_thread_num());
    ...
    double calc_time = ts_stop/1000.0f;
#ifdef _PRECALC_

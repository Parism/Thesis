...
void send_recv_init(double* av,int up,int down,int left,int right,MPI_Comm new_comm,MPI_Datatype col_type,MPI_Request* reqs,int *tags){

	fprintf(stderr, "PROCESS = %d\n",up );

	MPI_Send_init( av+NMAX+2+1, NMAX, MPI_DOUBLE, up, tags[1], new_comm, reqs); //send first row of initial array
	MPI_Send_init( av+(NMAX+2)*(NMAX)+1, NMAX, MPI_DOUBLE, down, tags[0], new_comm, reqs+1 );  //send last row of initial array
	MPI_Send_init( av+NMAX+2+1, NMAX, col_type, left, tags[3], new_comm, reqs+2 );			//send left column 
	MPI_Send_init( av+NMAX+2+NMAX, NMAX, col_type, right, tags[2], new_comm, reqs+3 );		//send right column

	MPI_Recv_init( av+1, NMAX, MPI_INT, up, tags[0], MPI_COMM_WORLD, reqs+4 );		
	MPI_Recv_init( av+(NMAX+2)*(NMAX+1)+1, NMAX, MPI_INT, down, tags[1], MPI_COMM_WORLD, reqs+5 );
 	MPI_Recv_init( av+NMAX+2+1, NMAX, MPI_INT, left, tags[2], MPI_COMM_WORLD, reqs+6 );
 	MPI_Recv_init( av+NMAX+2+NMAX+1, NMAX, MPI_INT, right, tags[3], MPI_COMM_WORLD, reqs+7 );
...
//	}
		fprintf(stderr, "EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE\n" );
	//if(*(reqs+4) > 0){
...
//	}
		fprintf(stderr, "teloooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooos\n" );

		return;
}

regular_array<NMAX+2> arr_u;
#ifdef _PRECALC_
regular_array<NMAX+2> arr_ffh, arr_ggh;
#else
regular_array<NMAX+2> arr_l, arr_r, arr_t, arr_b;
#endif
regular_array<NMAX+2> arr_w;

redblack_array<NMAX+2> ro_u;
#ifdef _PRECALC_
redblack_array<NMAX+2> ro_ffh, ro_ggh;
#else
redblack_array<NMAX+2> ro_l, ro_r, ro_t, ro_b;
#endif
redblack_array<NMAX+2> ro_w;
...
//  double C_E, C_W, C_N, C_S, g1, g2;
  ...
  MPI_Type_vector(NMAX,1,NMAX+2,MPI_FLOAT,&col_type);
...
//	  printf("limit %d\n", sx_limit);
      ...
      int threads_per_line = sqrt(num_threads);

      if(up != MPI_PROC_NULL){  //if there is block above
      	if(omp_get_thread_num()%threads_per_line == 0){//if this thread is in the first line
      		x_start_offset = -1;
      	}
      }

      if(down != MPI_PROC_NULL){  //if there is block beneath
      	if(omp_get_thread_num()%(threads_per_line) == (threads_per_line-1)){//if this thread is in the last line
      		x_end_offset = 1;
      	}
      }
      
      if(left != MPI_PROC_NULL){  //if there is block on the left
      	if(omp_get_thread_num() < threads_per_line){//if this thread is in the first column
      		y_start_offset = -1;
      	}
      }

      if(right != MPI_PROC_NULL){  //if there is block on the right
      	 if(omp_get_thread_num() >= (num_threads - threads_per_line)){//if this thread is in the last column
      	 	y_end_offset = 1;
      	 }
      }
...
//	  printf("Thread %d dimensions %d x %d, data block %d x %d, last offset x=%d y=%d\n", omp_get_thread_num(), sx, sy, bs_x, bs_y, lastoffset_x, lastoffset_y);
	  int start_pos_x = 1+bs_x * (omp_get_thread_num()%sx)+ x_start_offset,
		  start_pos_y = 1+bs_y * (omp_get_thread_num()/sx)+ y_start_offset,
		  end_pos_x   = start_pos_x+bs_x+(omp_get_thread_num()%sx==sx-1?lastoffset_x:0) + x_end_offset,
		  end_pos_y   = start_pos_y+bs_y+(omp_get_thread_num()/sx==sy-1?lastoffset_y:0) + y_end_offset;
...
/*#pragma omp master
	...
	send_recv_init(arr_u._get_data(),up,down,left,right,new_comm,col_type,&req_array_u[0][0],tag_array_u);
	...
	send_recv_init(arr_ffh._get_data(),up,down,left,right,new_comm,col_type,&req_array_ffh[0][0],tag_array_ffh);
	send_recv_init(arr_ggh._get_data(),up,down,left,right,new_comm,col_type,&req_array_ggh[0][0],tag_array_ggh);
#else
	...
	send_recv_init(arr_l._get_data(),up,down,left,right,new_comm,col_type,&req_array_l[0][0],tag_array_l);
	send_recv_init(arr_r._get_data(),up,down,left,right,new_comm,col_type,&req_array_r[0][0],tag_array_r);
	send_recv_init(arr_t._get_data(),up,down,left,right,new_comm,col_type,&req_array_t[0][0],tag_array_t);
	send_recv_init(arr_b._get_data(),up,down,left,right,new_comm,col_type,&req_array_b[0][0],tag_array_b);
#endif
	int tag_array_w[] = {20,21,22,23};
	MPI_Request req_array_w[2][4];
	send_recv_init(arr_w._get_data(),up,down,left,right,new_comm,col_type,&req_array_w[0][0],tag_array_w);
